...
import { FaEye, FaDownload, FaFileInvoiceDollar, FaShippingFast, FaBuilding, FaCalendar } from 'react-icons/fa';
...
const BusinessInvoices = () => {
  ...
  const fetchInvoices = async () => {
    try {
      console.log('🔍 BusinessInvoices: Starting to fetch transaction invoices...');
      setLoading(true);
      
      const data = await api.get('/api/business/transaction-invoices');
      console.log('📊 BusinessInvoices: Raw API response:', data);
      console.log('📊 BusinessInvoices: Data type:', typeof data);
      console.log('📊 BusinessInvoices: Is array:', Array.isArray(data));
      console.log('📊 BusinessInvoices: Data length:', data?.length);
      
      const invoicesArray = Array.isArray(data) ? data : [];
      console.log('📊 BusinessInvoices: Final invoices array:', invoicesArray);
      console.log('📊 BusinessInvoices: Setting invoices with length:', invoicesArray.length);
      
      setInvoices(invoicesArray);
    } catch (error) {
      console.error('BusinessInvoices: Error fetching transaction invoices:', error);
      console.error('BusinessInvoices: Error details:', {
      ...
      setLoading(false);
      console.log('BusinessInvoices: Fetch completed, loading set to false');
    }
  };

  const handleViewInvoice = async (invoiceId) => {
    try {
      const invoice = await api.get(`/api/business/transaction-invoices/${invoiceId}`);
  ...
  console.log('🎨 BusinessInvoices: Render state:', {
    loading,
  ...
  if (loading) {
    console.log('🔄 BusinessInvoices: Component is in loading state');
    return (
  ...
  console.log('🎨 BusinessInvoices: Rendering component with invoices:', invoices);
  console.log('🎨 BusinessInvoices: Invoices length:', invoices.length);
  console.log('🎨 BusinessInvoices: Loading state:', loading);
    ...
    <div className="bg-white p-6 md:p-8 rounded-lg shadow-md w-full">
      ...
      {invoices.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-gray-500">Your completed transaction invoices will appear here when payments are verified and approved.</p>
        </div>
      ) : (
        <>
          {/* Controls */}
            ...
            <div ref={invoiceRef} className="p-4" style={{ backgroundColor: '#ffffff', color: '#000000', maxWidth: '794px', margin: '0 auto', fontFamily: 'Arial, sans-serif', lineHeight: '1.5' }}>
              <div className="bg-white">
                ...
                <div className="flex justify-between items-start mb-6">
                  <div className="flex items-center gap-3">
                    {selectedInvoice.company_logo ? (
                      ...
                      <div className="w-32 h-32 bg-[#bca142] rounded-lg flex items-center justify-center">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                  <div>
                    <h3 className="font-semibold mb-2" style={{ color: '#1F2937', fontSize: '14px', fontWeight: '600' }}>Payment Made To:</h3>
                      ...
                      <tr>
                        <td className="p-2" style={{ border: '1px solid #D1D5DB', padding: '8px', verticalAlign: 'top' }}>
                          <div>
                            ...
                            <p className="text-xs" style={{ color: '#4B5563', fontSize: '11px', marginBottom: '4px' }}>Quote #{selectedInvoice.quote_id} - {selectedInvoice.shipping_mode} Shipping</p>
