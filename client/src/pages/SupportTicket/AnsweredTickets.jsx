...
const AnsweredTickets = () => {
  ...
  const fetchTickets = async () => {
    try {
      ...
      // Filter only answered tickets
      const answeredTickets = data.filter(ticket => ticket.status === 'answered');
  ...
  const SortableHeader = ({ children, sortKey }) => (
    <th className="py-4 px-4 text-left font-semibold cursor-pointer hover:bg-blue-100 transition-colors duration-200" onClick={() => handleSort(sortKey)}>
      <div className="flex items-center">
        {children}
        {sortConfig.key === sortKey ? (
          sortConfig.direction === 'ascending' ? 
            <FiChevronUp className="ml-1 text-blue-600" /> : 
            <FiChevronDown className="ml-1 text-blue-600" />
        ) : null}
      </div>
    </th>
  );
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-screen bg-gradient-to-br from-blue-50 via-cyan-50 to-blue-100">
        <div className="text-center">
          <div className="relative">
            <div className="animate-spin rounded-full h-16 w-16 border-4 border-blue-200 border-t-blue-600 mx-auto"></div>
            <FiMessageSquare className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-blue-600 animate-pulse" size={24} />
          </div>
          <p className="mt-4 text-lg font-medium text-gray-700 animate-pulse">Loading Answered Tickets...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-center h-screen bg-gradient-to-br from-blue-50 via-cyan-50 to-blue-100">
        <div className="text-center">
          ...
          <button 
            ...
            className="bg-gradient-to-r from-blue-500 to-cyan-600 text-white px-6 py-3 rounded-xl font-semibold hover:from-blue-600 hover:to-cyan-700 transition-all duration-300"
          >
            Try Again
          </button>
        </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-cyan-50 to-blue-100 p-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-white/80 backdrop-blur-lg rounded-2xl shadow-xl border border-white/20 p-8 mb-8">
          <div className="flex flex-col lg:flex-row justify-between items-center">
            <div className="flex items-center space-x-4 mb-4 lg:mb-0">
              ...
              <div>
                <h1 className="text-3xl font-bold bg-gradient-to-r from-blue-800 to-cyan-600 bg-clip-text text-transparent">
                  Answered Tickets
                </h1>
                <p className="text-gray-600 mt-1">Tickets with admin responses & solutions</p>
              </div>
            </div>
            <button 
              onClick={fetchTickets}
              className="flex items-center space-x-2 bg-gradient-to-r from-blue-500 to-cyan-600 text-white font-semibold py-3 px-6 rounded-xl hover:from-blue-600 hover:to-cyan-700 transition-all duration-300 transform hover:scale-105 shadow-lg"
            >
              <FiRefreshCw size={20} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {[
            { label: 'Total Answered', value: tickets.length, icon: FiMessageSquare, color: 'from-blue-500 to-cyan-500' },
        ...
        <div className="bg-white/80 backdrop-blur-lg rounded-2xl shadow-xl border border-white/20 p-6 mb-8">
          <div className="flex flex-col lg:flex-row items-center justify-between space-y-4 lg:space-y-0 lg:space-x-6">
            ...
            <div className="relative flex-1 max-w-md">
              ...
              <input
                type="text"
                placeholder="Search answered tickets..."
                value={searchTerm}
        ...
        <div className="bg-white/80 backdrop-blur-lg rounded-2xl shadow-xl border border-white/20 overflow-hidden">
          {sortedTickets.length === 0 ? (
            <div className="text-center py-16">
              <div className="p-4 bg-gradient-to-r from-blue-100 to-cyan-200 rounded-full w-24 h-24 mx-auto mb-6 flex items-center justify-center">
                <FiMessageSquare className="text-blue-600" size={48} />
              </div>
              <h3 className="text-xl font-semibold text-gray-900 mb-2">
                {searchTerm ? 'No Matching Answered Tickets' : 'No Answered Tickets Found'}
              </h3>
              <p className="text-gray-500 max-w-md mx-auto">
                {searchTerm 
                  ? `No answered tickets match your search criteria "${searchTerm}"`
                  : 'There are no answered tickets at the moment. Tickets with admin responses will appear here.'
                }
              </p>
            </div>
          ) : (
                ...
                <table className="w-full min-w-[800px]">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {paginatedTickets.map((ticket, index) => (
                      <tr key={ticket.id} className={`hover:bg-gradient-to-r hover:from-blue-50/50 hover:to-cyan-50/50 transition-all duration-300 ${
              ...
              <div className="bg-gradient-to-r from-blue-50 to-cyan-50 px-6 py-4 border-t border-gray-200">
                <div className="flex flex-col sm:flex-row items-center justify-between space-y-4 sm:space-y-0">
                  <div className="text-sm text-gray-700">
                    Showing <span className="font-bold text-blue-600">{sortedTickets.length > 0 ? startEntry : 0}</span> to{' '}
                    <span className="font-bold text-blue-600">{endEntry}</span> of{' '}
                    <span className="font-bold text-blue-600">{sortedTickets.length}</span> results
                  </div>
      ...
      {isDetailsModalOpen && selectedTicket && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white/95 backdrop-blur-lg rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-white/20">
            <div className="bg-gradient-to-r from-blue-500 via-cyan-600 to-blue-500 px-8 py-6 flex items-center justify-between">
              <div className="flex items-center space-x-4">
                <div className="p-3 bg-white/20 rounded-xl">
                  <FiMessageSquare className="text-white" size={24} />
                </div>
                <div>
                  <h2 className="text-2xl font-bold text-white">Answered Ticket {selectedTicket.ticket_number}</h2>
                  <p className="text-blue-100">Ticket with admin response & solution</p>
                </div>
              </div>
              <button
                onClick={closeModal}
                ...
                <div className="space-y-6">
                  <div className="bg-gradient-to-r from-blue-50 to-cyan-50 p-6 rounded-xl border border-blue-200">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="inline-flex items-center px-3 py-1 text-xs font-bold rounded-full bg-blue-100 text-blue-800">
            ...
            <div className="bg-gradient-to-r from-blue-50 to-cyan-50 px-8 py-6 flex justify-end border-t border-gray-200">
              <button
                onClick={closeModal}
                className="bg-gradient-to-r from-blue-500 to-cyan-600 hover:from-blue-600 hover:to-cyan-700 text-white px-6 py-3 rounded-xl font-semibold transition-all duration-300 transform hover:scale-105 shadow-lg"
              >
                Close Details
              </button>
            </div>
