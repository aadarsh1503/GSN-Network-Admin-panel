...
import ReplyTicket from './ReplyTicket'; // Reusing the same detail page component

// --- Reusable Components (Reused for consistency) ---
...
const StatusBadge = ({ status }) => {
  ...
  if (status.toLowerCase() === 'closed') {
    // Styling specifically for the 'closed' status to match the screenshot
    return <span className={`${baseStyle} bg-slate-500 text-white`}>{status}</span>;
  }
...
// --- Mock Data (Only contains closed tickets) ---
const closedTicketsData = [
  { id: 1, subject: '343535', submittedBy: '', status: 'closed' },
  // Add more closed tickets here if you need to test with more data
];
...
const ClosedTickets = () => {
  ...
  // Otherwise, render the list of closed tickets
  return (
    <div className="p-4 sm:p-6 lg:p-8 bg-gray-50 min-h-screen">
      <div className="max-w-7xl mx-auto bg-white p-6 rounded-lg shadow-md">
        <h1 className="text-2xl font-bold text-gray-800 mb-6">Closed Tickets</h1>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mt-4 text-sm text-gray-600">
          <p>Showing 1 to {closedTicketsData.length} of {closedTicketsData.length} entries</p>
