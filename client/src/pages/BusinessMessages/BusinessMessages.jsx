...
const BusinessMessages = () => {
  ...
  useEffect(() => {
    ...
    console.log('🔍 BusinessMessages component loaded');
    ...
    if (user.role !== 'business') {
      console.log('❌ User is not a business user, role:', user.role);
  ...
  // Separate useEffect to handle auto-selection after conversations are loaded
  useEffect(() => {
    const { selectedCompanyId, selectedCompanyName } = location.state || {};
    
    if (selectedCompanyId && conversations.length > 0 && !selectedConversation) {
      console.log('🎯 Auto-selecting company:', selectedCompanyName, 'ID:', selectedCompanyId);
      
      // Find the conversation with the specified company
      const targetConversation = conversations.find(conv => conv.other_user_id === selectedCompanyId);
      
      if (targetConversation) {
        console.log('✅ Found existing conversation with company');
        handleSelectConversation(targetConversation);
      } else {
        console.log('📝 No existing conversation found, creating new conversation context');
        // Create a temporary conversation object for new conversation
        const newConversation = {
          userId: selectedCompanyId,
          userName: selectedCompanyName,
          other_user_id: selectedCompanyId,
          other_user_name: selectedCompanyName,
          other_user_role: 'company',
  ...
  const fetchConversations = async () => {
    setLoading(true);
    try {
      console.log('🔍 Fetching business conversations...');
      const data = await api.get('/api/business/messages/conversations');
      ...
      if (error.message.includes('Authentication failed') || error.message.includes('token')) {
        toast.error('Please login again to access messages');
  ...
  const fetchUnreadCount = async () => {
    try {
      ...
      const data = await api.get('/api/business/messages/unread-count');
  ...
  const fetchConversation = async (userId) => {
    ...
    try {
      ...
      const data = await api.get(`/api/business/messages/conversation/${userId}`);
      console.log('✅ Conversation fetched successfully:', data);
      setConversationMessages(Array.isArray(data) ? data : []);
    } catch (error) {
      console.error('❌ Error fetching conversation:', error);
      setConversationMessages([]);
      toast.error('Failed to fetch conversation: ' + error.message);
    } finally {
  ...
  const handleSelectConversation = async (conversation) => {
    ...
    if (conversation.unread_count > 0) {
      try {
        await api.put(`/api/business/messages/conversation/${conversation.other_user_id}/read`);
        fetchUnreadCount();
        fetchConversations();
      } catch (error) {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      await api.post('/api/business/messages/send', {
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="bg-white rounded-lg shadow p-6 mb-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 mb-2">Business Messages</h1>
            <p className="text-gray-600">Communicate with companies and clients</p>
          </div>
          {unreadCount > 0 && (
                          ...
                          <div className={`max-w-[80%] rounded-lg p-3 shadow-sm border-l-4 ${
                            ...
                            <div className="flex items-center gap-2 mb-2">
                              ...
                              <p className={`text-xs font-medium ${
                                ...
                                Admin - {msg.subject || `Ticket ${msg.ticket_number || 'Response'}`}
                              </p>
                              {msg.ticket_number && (
