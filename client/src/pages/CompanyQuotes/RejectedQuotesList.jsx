...
const RejectedCompanyQuotesList = () => {
  ...
  const fetchRejectedQuotes = async () => {
    try {
      ...
      // Filter for rejected quotes only
      const rejectedQuotes = data.filter(quote => {
        return quote.status === 'rejected';
      });
      
      console.log(`📊 Rejected quotes: ${rejectedQuotes.length}/${data.length}`);
      
      if (rejectedQuotes.length > 0) {
        // Debug first few quotes to understand data structure
        console.log('📊 Sample rejected quote data:');
        rejectedQuotes.slice(0, 2).forEach((quote, index) => {
      ...
      setQuotes(Array.isArray(rejectedQuotes) ? rejectedQuotes : []);
    } catch (error) {
      console.error('Error fetching rejected quotes:', error);
      toast.error('Failed to fetch rejected quotes');
      setQuotes([]);
    } finally {
      setLoading(false);
    }
  ...
  const getStatusBadge = (status) => {
    ...
    return (
      <span className={`px-1 py-0.5 rounded text-xs font-medium ${colors[status] || 'bg-red-100 text-red-800'}`}>
      ...
      <div className="max-w-full mx-auto">
        ...
        <div className="bg-white p-3 rounded-lg shadow-sm">
          <div className="flex justify-between items-center mb-3">
            <h2 className="text-lg font-semibold text-gray-800">Rejected Quotes ({filteredQuotes.length})</h2>
                ...
                {currentItems.length === 0 ? (
                  <tr>
                    <td colSpan="10" className="px-2 py-6 text-center text-gray-500 text-sm">
                      {quotes.length === 0 ? 'No rejected quotes found.' : 'No quotes match your current filters.'}
                    </td>
                  </tr>
                ) : (
                    ...
                    <tr key={quote.id} className="bg-white border-b hover:bg-gray-50">
                      ...
                      <td className="px-2 py-2">
                        {quote.accepted_price && quote.accepted_price !== 'null' ? (
                          <div className="min-w-[120px]">
                            <div className="font-bold text-xs text-red-600">${quote.accepted_price}</div>
                            <div className="text-xs text-gray-500 truncate">{quote.accepted_transit_time || 'N/A'}</div>
                            <div className="text-xs text-red-600">Not Accepted</div>
                          </div>
                        ) : (
