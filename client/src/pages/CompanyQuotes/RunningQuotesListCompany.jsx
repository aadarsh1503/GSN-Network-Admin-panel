...
const RunningQuotesListCompany = () => {
  const [runningQuotes, setRunningQuotes] = useState([]);
  ...
  const fetchRunningQuotes = async () => {
    try {
      ...
      // Filter for running quotes
      const running = data.filter(quote => quote.status === 'running');
      setRunningQuotes(Array.isArray(running) ? running : []);
    } catch (error) {
  ...
  const filteredQuotes = runningQuotes.filter(quote =>
      ...
      <div className="max-w-6xl mx-auto bg-white p-6 rounded-lg shadow-sm">
              ...
              {paginatedQuotes.length === 0 ? (
                <tr className="border-b">
                  <td colSpan="15" className="text-center py-10 text-gray-500">
                    {searchTerm ? 'No running quotes found matching your search.' : 'No running quotes available.'}
                  </td>
                </tr>
              ) : (
                    ...
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-2">
                        <button 
                          ...
                          className="p-2 bg-gradient-to-r from-green-500 to-green-600 text-white rounded-lg hover:from-green-600 hover:to-green-700 transition-all duration-200 shadow-md hover:shadow-lg transform hover:scale-105"
                          title="View Details"
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mt-4 text-sm text-gray-600 gap-4">
          <div>
            ...
            {searchTerm && ` (filtered from ${runningQuotes.length} total entries)`}
          </div>
          <div className="flex items-center">
            <button 
