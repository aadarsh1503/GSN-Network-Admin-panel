...
const ClosedCompanyQuote = () => {
  ...
  const fetchClosedQuotes = async () => {
    try {
      ...
      // Filter for closed quotes only
      const closedQuotes = data.filter(quote => {
        return quote.status === 'closed';
      });
      
      console.log(`📊 Closed quotes: ${closedQuotes.length}/${data.length}`);
      
      if (closedQuotes.length > 0) {
        // Debug first few quotes to understand data structure
        console.log('📊 Sample closed quote data:');
        closedQuotes.slice(0, 2).forEach((quote, index) => {
      ...
      setQuotes(Array.isArray(closedQuotes) ? closedQuotes : []);
    } catch (error) {
      console.error('Error fetching closed quotes:', error);
      toast.error('Failed to fetch closed quotes');
      setQuotes([]);
    } finally {
      setLoading(false);
    }
      ...
      <div className="max-w-full mx-auto">
        ...
        <div className="bg-white p-3 rounded-lg shadow-sm">
          <div className="flex justify-between items-center mb-3">
            <h2 className="text-lg font-semibold text-gray-800">Closed Quotes ({filteredQuotes.length})</h2>
                ...
                {currentItems.length === 0 ? (
                  <tr>
                    <td colSpan="10" className="px-2 py-6 text-center text-gray-500 text-sm">
                      {quotes.length === 0 ? 'No closed quotes found.' : 'No quotes match your current filters.'}
                    </td>
                  </tr>
                ) : (
                    ...
                    <tr key={quote.id} className="bg-white border-b hover:bg-gray-50">
                      ...
                      <td className="px-2 py-2">
                        {quote.accepted_price && quote.accepted_price !== 'null' ? (
                          <div className="min-w-[120px]">
                            <div className="font-bold text-xs text-gray-600">${quote.accepted_price}</div>
                      ...
                      <td className="px-2 py-2">
                        <div className="flex items-center">
                          <button 
                            ...
                            className="p-1.5 bg-gradient-to-r from-gray-500 to-gray-600 text-white rounded hover:from-gray-600 hover:to-gray-700 transition-all duration-200 shadow-sm hover:shadow-md"
                            title="View Details"
