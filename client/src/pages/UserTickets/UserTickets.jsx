...
const UserTickets = () => {
  ...
  const fetchTickets = async () => {
    try {
      setLoading(true);
      const response = await api.get('/api/user/help/tickets');
      setTickets(response);
    } catch (error) {
  ...
  const fetchCompanies = async () => {
    try {
      const response = await api.get('/api/user/my-companies');
  ...
  const handleTicketSubmit = async (e) => {
    ...
    if (ticketForm.recipient_type === 'company' && !ticketForm.recipient_id) {
    ...
    try {
      ...
      await api.post('/api/user/help/ticket', ticketForm);
                ...
                <div>
                  ...
                  {companies.length === 0 && (
                    <p className="text-sm text-gray-500 mt-1">
                      No companies found. You need to have received quotes from companies to send them tickets.
                    </p>
                  )}
                </div>
              )}
