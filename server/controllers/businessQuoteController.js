// controllers/businessQuoteController.js
// Business user quote management - same functionality as userQuoteController.js
...
// @desc    Get business user's quotes with responses
// @route   GET /api/business-quotes/my-quotes
// @access  Private/Business
const getBusinessUserQuotes = async (req, res) => {
    const businessId = req.user.id;

    try {
        // Use the same logic as company quotes to ensure consistent status display
        const sql = `
            SELECT q.*, 
                   COUNT(qr.id) as response_count,
                   MIN(qr.price) as lowest_price,
                   MAX(qr.price) as highest_price,
                   -- Get the actual quote status from the database
                   -- This ensures business panel shows same status as company panel
        ...
        // Log for debugging
        console.log(`📊 [BUSINESS-QUOTES] Fetched ${rows.length} quotes for business user ${businessId}`);
        rows.slice(0, 3).forEach(quote => {
            console.log(`  Quote #${quote.id}: Status="${quote.status}", Responses=${quote.response_count}`);
        });
        
        res.status(200).json(rows);
    } catch (error) {
        console.error('Error fetching business user quotes:', error);
...
const getBusinessQuoteResponses = async (req, res) => {
    ...
    try {
        // First verify the quote belongs to the business user
        ...
        const [responses] = await db.execute(sql, [businessId, quoteId]);
...
// @route   POST /api/business-quotes/accept-response
// @access  Private/Business
const acceptBusinessQuoteResponse = async (req, res) => {
    ...
    try {
        ...
        // Verify the quote belongs to the business user
        const [quoteRows] = await db.execute(
        ...
        // Check if business user has already accepted any response for this quote (with FOR UPDATE to prevent race conditions)
        ...
        // Check if business user has already responded to this specific quote response
        const [existingResponse] = await db.execute(
            ...
            [quoteId, businessId, quoteResponseId]
        );
        ...
        if (existingResponse.length > 0) {
            ...
            await db.execute(
                ...
                ['accepted', quoteId, businessId, quoteResponseId]
            );
        } else {
            ...
            await db.execute(
                `INSERT INTO user_quote_status (quote_id, user_id, company_id, quote_response_id, status, accepted_at) 
                 VALUES (?, ?, ?, ?, 'accepted', NOW())`,
                [quoteId, businessId, companyId, quoteResponseId]
            );
        }
        ...
        // Get company and business user details for email notification
        ...
        try {
            if (companyDetails.length > 0 && businessDetails.length > 0) {
        ...
        try {
            ...
            await realTimeNotificationService.notifyQuoteStatusChange(
                ...
                'approved', 
                'business',
                { accepted_by: businessDetails[0].name }
            );
        } catch (realtimeError) {
...
// @route   POST /api/business-quotes/reject-response
// @access  Private/Business
const rejectBusinessQuoteResponse = async (req, res) => {
    const { quoteId, quoteResponseId, companyId } = req.body;
    const businessId = req.user.id;
...
// @desc    Get business user notifications
// @route   GET /api/business-quotes/notifications
// @access  Private/Business
const getBusinessUserNotifications = async (req, res) => {
    const businessId = req.user.id;
    ...
    try {
        const sql = `
            ...
            WHERE un.user_id = ? AND (n.target_audience = 'all' OR n.target_audience = 'businesses')
        ...
        console.error('Error fetching business user notifications:', error);
        res.status(500).json({ message: 'Server error fetching notifications' });
    }
};

// @desc    Mark notification as read
// @route   PUT /api/business-quotes/notifications/:id/read
// @access  Private/Business
...
// @desc    Update quote status (Business user can change their own quote status)
// @route   PUT /api/business-quotes/:quoteId/status
// @access  Private/Business
const updateBusinessQuoteStatus = async (req, res) => {
    const { quoteId } = req.params;
...
// @route   GET /api/business-quotes/:quoteId/status
// @access  Private/Business
const getBusinessQuoteStatus = async (req, res) => {
    const { quoteId } = req.params;
    const businessId = req.user.id;
    ...
    try {
        ...
        // Get company that updated the status (if any recent updates)
        const [statusUpdateInfo] = await db.execute(`
            SELECT 
                c.name as company_name,
                c.email as company_email,
                qr.created_at as response_date,
                q.updated_at as last_status_update
            FROM quotes q
            LEFT JOIN quote_responses qr ON q.id = qr.quote_id
        ...
        res.status(200).json({
            ...
            userResponses,
            statusUpdateInfo: statusUpdateInfo.length > 0 ? statusUpdateInfo[0] : null
        });
...
// @desc    Get business user dashboard stats
// @route   GET /api/business-quotes/dashboard-stats
// @access  Private/Business
const getBusinessUserDashboardStats = async (req, res) => {
    const businessId = req.user.id;
